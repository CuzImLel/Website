...
const Repository: React.FC<props> = ({ data }) => {
  ...
  const [languageColors, setLanguageColors] = useState<ColorLang[]>([{language: "null", color: "null"}]);
  
    
    const colorUrl = 'https://raw.githubusercontent.com/ozh/github-colors/master/colors.json';
    ...
    useEffect(() => {
      fetch(colorUrl)
        ...
        .then((colorsData) => {
          ...
          const colorsArray: ColorLang[] = Object.keys(colorsData).map((language) => ({
            language,
...
bookmarks
        ...
        <div className="repo_footer">
          <div className="lang_circle" style={{  backgroundColor: languageColors.find((colorLang) => colorLang.language === repo.language)?.color || 'green'}}></div>
